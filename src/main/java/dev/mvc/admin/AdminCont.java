...
//      session.setAttribute("admin_id", adminVO.getId());
...
//      
//    } else { // 로그인 실패
//      // mav.setViewName("/admin/login_fail_msg");  X  POST방식에서는 jsp에서 c import가 인식이 안 됨(실행이 안 됨)
  ...
  */
  ...
  public ModelAndView login_cookie_proc(
    ...
    if (cnt == 1) { // 로그인 성공
      ...
      AdminVO adminVO_read = adminProc.read_by_id(adminVO.getId());
